// Paste this into Google Apps Script (Extensions → Apps Script from inside the Sheet),
// then Deploy → New deployment → type "Web app" → execute as Me,
// access: pick whatever your account allows. Copy the /exec URL into SHEET_WEBHOOK_URL.
//
// Sheet must exist and have a tab named "Messages" (or change SHEET_NAME below).
